...
export default async function PrestadorHome() {
  ...
  const { data: open } = await supabase
    ...
    .select(
      "id, description, urgent, address, estimated_price, status, lat, lng, created_at, category:service_categories(name, icon), client:profiles!service_requests_client_id_fkey(full_name, city)",
    )
